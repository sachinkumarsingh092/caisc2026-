...
while preserving every protocol-level correctness invariant. Taking
\texttt{etcd-io/raft}---the most widely deployed Raft implementation,
underpinning Kubernetes, CockroachDB, and the etcd key-value store---as
our target, we compose a layered verification harness that combines
the library's in-tree TLA$^{+}$ trace-validation infrastructure, a
purpose-built deterministic simulation tester running on top of
upstream's pluggable \texttt{RawNode}/\texttt{Storage} interfaces, and
a Porcupine-based linearizability checker on recorded client histories
under partition, drop, reorder, and duplicate fault injection. The
harness is then used as the rejection oracle for an evolutionary
search driven by a frontier coding model: each candidate must clear
upstream compilation, the unmodified upstream test suite, a seeded
multi-scenario fault sweep, and a Raft-protocol invariant checker
before its benchmark score is allowed to compete. We report (i)~the
design and injection points of the harness against existing upstream
verification assets, (ii)~the calibration of its noise floors, (iii)~
the candidates surfaced by an evolutionary loop targeting the
leader-side flow-control function \texttt{maybeSendAppend}, and (iv)~an
ablation showing which harness layer caught which class of incorrect
candidate. Our central claim is methodological: in a domain with
mechanically checkable safety properties, the harness---not the model---
is the load-bearing artefact, and a sufficiently sharp harness lets a
general-purpose coding agent search the algorithmic neighbourhood of
a mature systems codebase safely.
\end{abstract}
...
most contemporary distributed systems---etcd, Kubernetes, CockroachDB,
Docker Swarm, TiKV. Implementations of Raft are required to preserve
...
the LLM is forced through a mechanical pipeline---compile, full upstream
test suite, seeded deterministic fault-injection sweep, linearizability
check, benchmark---before any performance score is reported, the
...
This paper instantiates that pattern over \texttt{etcd-io/raft} at a
pinned commit. Our contributions are:
\begin{enumerate}
  \item A minimal-upstream-patch DST harness over the library's
    existing \texttt{RawNode}/\texttt{Storage} interfaces, with
    seeded fault injection (partition, heal, drop, reorder,
    duplicate) and a two-layer oracle (Raft-protocol invariants and
    Porcupine linearizability on client histories). The harness
    achieves bit-for-bit reproducibility per seed after a four-line
    patch to the upstream election-timeout RNG.
  ...
  \item An evolutionary-search wrapper that uses the harness as the
    ...
    \texttt{(r~*raft) maybeSendAppend}; candidates that fail to
...
engine. Verification-gated autonomous
...
consensus library: the harness is empirical (deterministic simulation
plus linearizability checking) rather than formal, and the search
target is a single function inside a $\sim$85~KB host file.
...
\texttt{Config.Rand func(int) int} field that, when set, replaces a
...
\texttt{RawNode}. The cluster's per-tick step performs, in order:
(1)~tick all non-partitioned nodes; (2)~for each node, drain
\texttt{Ready()}, persist \texttt{Entries} to a per-node
\texttt{MemoryStorage}, apply \texttt{CommittedEntries} to a
single-register state machine, sort outbound \texttt{Messages} into a
canonical $\langle\text{To},\text{From},\text{Type},\text{Term},
\text{Index}\rangle$ order before sending (sorting is required
because upstream builds \texttt{r.msgs} by iterating a
\texttt{map[uint64]*Progress}), then \texttt{Advance(rd)};
(3)~deliver pending inbox messages by \texttt{Step()}-ing them into
target nodes; (4)~poll pending client read operations against the
linearizable safe index. All randomness is sourced from a single
seeded \texttt{*rand.Rand} threaded through both the harness and
upstream's election timeout.
...
\verb|[tag=0x01][opID:8][value:8]| so committed entries can be matched
...
On commodity hardware (Apple M-class, single-process), the per-seed
cost of the full harness pyramid (compile, upstream test suite, 8
fault-injected DST seeds at 1500~ticks each, Porcupine check, and a
five-run benchmark) is approximately $30$~seconds wall-clock. The
DST layer alone runs at $\sim$12\,k ticks/second for a three-node
cluster; a 200-seed sweep at 2000~ticks completes in under four
minutes.
...
We target a single function in \texttt{raft.go}:
\begin{center}
\texttt{(r *raft) maybeSendAppend(to uint64, sendIfEmpty bool) bool}
\end{center}
This is the per-follower decision point that gates flow control,
batching, pipelining, and snapshot fallback. It is the single hottest
function on the leader's outbound path, $\sim$45~lines of code with
all the relevant levers in scope: \texttt{pr.IsPaused()},
\texttt{pr.Inflights.Full()}, \texttt{r.raftLog.entries(\allowbreak
pr.Next,\allowbreak{} r.maxMsgSize)}, the \texttt{StateReplicate}
predicate, and the post-send progress update via
\texttt{pr.SentEntries} and \texttt{pr.SentCommit}. The function is
marked with \texttt{\#~EVOLVE-BLOCK-START} and \texttt{\#~EVOLVE-BLOCK-END}
comments; candidates that touch the file outside this region are
rejected at diff-application time.

The candidate generator is instructed by system prompt to preserve
five named contracts: (i)~the function signature; (ii)~the
\texttt{IsPaused} short-circuit; (iii)~the snapshot fallback path on
\texttt{term()} error; (iv)~the post-send progress updates;
(v)~prohibition on new package-level state, goroutines, locks, or
I/O.
...
compilation; \emph{(b)}~the unmodified upstream test suite for the
affected packages; \emph{(c)}~the DST sweep ($N=8$ seeds, 1500~ticks
each, full fault catalogue); \emph{(d)}~the linearizability check on
the recorded client histories; only then \emph{(e)}~the benchmark.
Any layer's failure terminates evaluation immediately and is recorded
as the candidate's rejection reason. The harness layers contribute
their failure modes independently---a candidate that compiles
correctly but violates log-matching is caught at (c), while a
candidate that passes (c) but has reordered \texttt{Ready.Messages}
in a way that breaks linearizability is caught at (d).
...
working copy of \texttt{raft.go} on each evaluation. The upstream
test suite is run on every candidate before any benchmark.
...
The benchmark exhibits a bimodal ns/op distribution under repeated
runs of an unmodified binary (typical mode at $\sim$2.3--2.5\,$\mu$s/op,
secondary mode at $\sim$3.5--6.5\,$\mu$s/op driven by system load);
we report the minimum across five \texttt{-benchtime=1s} runs.

\subsection{Loop Output}
\label{sec:results-loop}
We ran the loop for $30$ iterations over $\sim$76 minutes of wall-clock
on a single host. Of $30$ proposed candidates, \emph{all $30$ cleared
the rejection oracle}: every candidate compiled, passed the full
upstream test suite, passed $8$ fault-injected DST seeds at
$1500$~ticks, and produced a non-violating client history. None were
...
The highest-scoring variant introduces four coordinated edits to
\texttt{maybeSendAppend}:
...
best-case throughput, governs end-to-end commit time. A more robust
evaluator scoring would use the paired median rather than the min;
we treat this as a methodological lesson and discuss it in
\S\ref{sec:limits}.

\subsection{Per-Layer Ablation}
\label{sec:results-ablation}
Across the $30$-iteration run, no candidate failed compilation,
no candidate failed the upstream test suite, and no candidate
triggered an invariant violation during the DST sweep. The
linearizability layer's noise-floor gate did not fire as a
rejection. This is not, on its own, evidence that the harness layers
are equivalent. To probe layer-uniqueness we re-evaluated the
rejected-elsewhere candidates from an earlier exploratory run
in which the prompt allowed unconstrained edits to the function
(no contract enumeration, no signature pinning). In that run we
observed three rejection classes:
\begin{itemize}
  \item \emph{Compilation rejections} (typed-method invention, e.g.,
    a candidate that called \texttt{pr.IsHealthy()} which does not
    exist) are by far the most common when the prompt does not
    enumerate the available methods.
  \item \emph{Upstream test rejections} catch contract violations
    (e.g., a candidate that removed the
    \texttt{pr.SentEntries(\ldots)} call). These are caught by
    targeted unit tests in \texttt{raft\_test.go} on the leader
    progress tracker; without the upstream suite, the DST sweep
    would still catch many---but not all---of them.
  \item \emph{Invariant rejections} from the DST sweep are the
    distinctive contribution of the harness: in our prior runs,
    a candidate that batched the \texttt{pr.SentCommit(\ldots)} call
    after \texttt{r.send(\ldots)} based on a guess about delivery
    ordering passed both compilation and the upstream test suite but
    failed log-prefix agreement under partition-with-reorder seeds.
\end{itemize}
The fully-constrained prompt used in the headline 30-iter run did
not surface invariant rejections, but our ablation supports the
necessity of the DST layer: targeted unit tests of the upstream
suite do not exhaustively cover the failure modes the DST sweep
finds.

\subsection{Cost}
\label{sec:results-cost}
The $30$-iteration loop completed in approximately $76$~minutes
...
and demonstrably misranked our best candidate; paired-distribution
scoring (per Table~\ref{tab:bench-paired}) is the more robust report
and should be the in-loop scoring function in future runs. The
underlying coding model is stochastic and closed-source; different
prompt templates, temperatures, or checkpoints will explore
different regions.
...
\texttt{Verdi}~\cite{verdi} (Coq) demonstrated machine-checked proofs
of distributed implementations at scale, and \texttt{Verus}~\cite{verus}
brings SMT-backed verification to Rust at lower proof cost.  Go has
no such analogue, which is why our Layer~1 is an existing
TLA$^{+}$ trace validator rather than a refinement proof.
...
framework~\cite{openevolve} across ten systems benchmarks; we narrow
it to a single safety-critical function.  Agent benchmarks like
\texttt{SWE-bench}~\cite{swebench} gate at the repository level
rather than the function-and-trace level we operate at.
...
checking; contemporary continuations include
\texttt{Antithesis}~\cite{antithesis}, \texttt{TigerBeetle}'s
\texttt{VOPR}~\cite{tigerbeetleDST}, and \texttt{Madsim}~\cite{madsim}.
Property-based testing contributes the underlying seeded-random
discipline~\cite{quickcheck,hypothesis}.  Our DST inherits FDB's
single-goroutine seeded-RNG style and the register-history checker
pattern of Knossos~\cite{knossos}/Porcupine~\cite{porcupine}.  The
\texttt{etcd} project ships a robustness suite targeting the
integrated server~\cite{etcdRobustness}; our harness is the
algorithm-library sibling.
