...
type Config struct {
	...
	// Rand, if non-nil, is used to pick the randomized election timeout
	// (see resetRandomizedElectionTimeout). If nil, a process-global
	// crypto/rand-backed source is used. Setting this field is required
	// for deterministic simulation testing of multiple raft groups, since
	// the default source is not seedable.
	Rand func(n int) int
}
...
type raft struct {
	...
	// intn, if non-nil, is the per-raft-group source for the randomized
	// election timeout. When nil, callers fall back to the process-global
	// crypto/rand-backed source. Populated from Config.Rand in newRaft.
	intn func(n int) int
}
...
func (r *raft) send(m *pb.Message) {
	...
	if m.GetType() == pb.MsgVote || m.GetType() == pb.MsgVoteResp || m.GetType() == pb.MsgPreVote || m.GetType() == pb.MsgPreVoteResp {
		if m.GetTerm() == 0 {
			...
			r.logger.Panicf("term should be set when sending %s", m.GetType())
		}
	} else {
		if m.GetTerm() != 0 {
			r.logger.Panicf("term should not be set when sending %s (was %d)", m.GetType(), m.GetTerm())
		}
...
func (r *raft) maybeSendAppend(to uint64, sendIfEmpty bool) bool {
	...
	// Fast path: in the batched-send loop (sendIfEmpty==false), once the
	// follower's pr.Next has caught up past the log tail, there is no point
	// doing the term lookup or the entries fetch. Compute lastIndex lazily
	// (only in this branch) to avoid paying for it on the entry-shipping
	// path.
	if !sendIfEmpty {
		if next > r.raftLog.lastIndex() && !pr.CanBumpCommit(commit) {
			return false
		}
	}
	...
	if err != nil {
		// The log probably got truncated at >= pr.Next, so we can't catch up
		// the follower log anymore. Send a snapshot instead.
		return r.maybeSendSnapshot(to, pr)
	}

	var ents []*pb.Entry
	// In a throttled StateReplicate only send empty MsgApp, to ensure
	// progress. Otherwise, if we had a full Inflights and all inflight
	// messages were in fact dropped, replication to that follower would
	// stall. Instead, an empty MsgApp will eventually reach the follower
	// (heartbeat responses prompt the leader to send an append), allowing it
	// to be acked or rejected, both of which will clear out Inflights.
...
func stepLeader(r *raft, m *pb.Message) error {
	...
	case pb.MsgSnapStatus:
		...
		if !m.GetReject() {
			pr.BecomeProbe()
			r.logger.Debugf("%x snapshot succeeded, resumed sending replication messages to %x [%s]", r.id, m.GetFrom(), pr)
		} else {
...
func stepCandidate(r *raft, m *pb.Message) error {
	...
	case pb.MsgTimeoutNow:
		r.logger.Debugf("%x [term %d state %v] ignored MsgTimeoutNow from %x", r.id, r.Term, r.state, m.GetFrom())
	}
	return nil
}
...
func (r *raft) handleAppendEntries(m *pb.Message) {
	...
	r.logger.Debugf("%x [logterm: %d, index: %d] rejected MsgApp [logterm: %d, index: %d] from %x",
		r.id, r.raftLog.zeroTermOnOutOfBounds(r.raftLog.term(m.GetIndex())), m.GetIndex(), m.GetLogTerm(), m.GetIndex(), m.GetFrom())
...
func (r *raft) resetRandomizedElectionTimeout() {
	intn := r.intn
	if intn == nil {
		intn = globalRand.Intn
	}
	r.randomizedElectionTimeout = r.electionTimeout + intn(r.electionTimeout)
}
