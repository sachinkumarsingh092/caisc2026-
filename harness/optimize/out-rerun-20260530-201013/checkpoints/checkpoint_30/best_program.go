...
func (r *raft) maybeSendAppend(to uint64, sendIfEmpty bool) bool {
	...
	// Fast path: when the caller does not want empty messages, only bother
	// computing prevTerm/entries if there is actually something new to send.
	// This avoids term() and entries() reads when nothing changed.
	if !sendIfEmpty {
		// If throttled, we can't send entries; only an empty MsgApp would be
		// possible, which the caller has opted out of.
		if throttled {
			return false
		}
		if pr.Next > r.raftLog.lastIndex() && !pr.CanBumpCommit(committed) {
			return false
		}
	}
	...
	if n := len(ents); n != 0 {
		pr.SentEntries(n, uint64(payloadsSize(ents)))
	} else {
		pr.SentEntries(0, 0)
	}
	pr.SentCommit(committed)
	return true
}
